...
- **`mv`** (`smallclueMvCommand`, `core.c`) — **still a genuine gap, not fixed by cp's upgrade.** `mv` calls `rename()` and, on `EXDEV`, falls back to `smallclueCopyFile` — the same single-file-only helper that still explicitly rejects directories. Unlike `cp`, `mv`'s cross-device fallback was never repointed at the newer recursive-copy path, so a cross-filesystem `mv` of a directory still fails today with the same "is a directory" error.
