...
- **`mount`/`umount`** — the **Linux code path is a genuine, generic `mount(2)`/`umount(2)` syscall wrapper**, not iOS-bind-mount-only (confirmed: zero special-casing, positive or negative, for `proc`/`tmpfs`/`sysfs`/`devtmpfs` anywhere — unrecognized `-o` tokens are passed straight through as filesystem-specific mount data exactly like real `mount(8)` does). This means `mount -t proc proc /proc` etc. **should already work** in principle, but it is untested in-repo and `umount` has **no `-l`/`-f`** (lazy/force unmount via `umount2()` flags — plain `umount()` only), which matters for cleanly tearing down busy pseudo-mounts during shutdown.
...
- **`rsync` — the README describes the wrong default engine.** By default (unless `PSCALI_RSYNC_LEGACY=1` is set), `smallclueRsyncCommand` (`core.c:17866-17869`) dispatches to the **vendored upstream `openrsync`** (`third-party/openrsync/`, built unconditionally per `CMakeLists.txt:60-181`) — a real rsync-protocol client, not the hand-rolled local-sync-plus-scp engine the README documents. The real default engine's actual gaps vs GNU rsync: **no `-u`/`--update` or `-c`/`--checksum` at all** (absent from openrsync's own option table, `third-party/openrsync/main.c:303-441`), `--link-dest`/`--copy-dest` compiled out (`#if 0`), `-z`/`--compress` is cosmetic (only requests ssh transport compression, not rsync-protocol payload compression — commented as a known limitation in `main.c:437-442`), no `--progress`, and daemon-push (uploading to a remote `rsync://`/`::` target) is explicitly rejected ("daemon receivers not yet supported", `main.c:198`). The legacy hand-rolled engine (only reachable via the undocumented `PSCALI_RSYNC_LEGACY=1` escape hatch) is the one that actually matches the README's description.
...
- **`nslookup`/`host`** — thin `getaddrinfo`/`getnameinfo` wrappers; no MX/TXT/NS/SRV record queries, no reverse/PTR lookup, and no ability to query a non-default DNS server (the "server" positional argument is accepted and silently ignored with a warning).
- **`ip addr`** — read-only "show" only; no `add`/`del`/`flush`/`ip link`/`ip route` at all (pure `getifaddrs()`, no netlink socket).
