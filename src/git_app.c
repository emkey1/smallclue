...
static int smallclueGitCommandRemote(git_repository *repo, int argc, char **argv) {
    ...
    if (strcmp(sub, "show") == 0) {
        ...
        for (int i = 0; i < subargc; ++i) {
            ...
            if (name_count >= (sizeof(names) / sizeof(names[0]))) {
                smallclueGitPrintError("too many remote show names");
                return 2;
            }
            names[name_count++] = arg;
        }
        ...
        if (name_count == 0) {
            ...
            if (git_remote_list(&remote_names, repo) != 0) {
                smallclueGitPrintLibgitError("remote show list failed");
                return 1;
            }
            if (remote_names.count > 1) {
                qsort(remote_names.strings, remote_names.count, sizeof(char *), smallclueGitCompareCStringPtr);
            }
            for (size_t i = 0; i < remote_names.count; ++i) {
                if (remote_names.strings[i] && *remote_names.strings[i]) {
                    puts(remote_names.strings[i]);
                }
            }
