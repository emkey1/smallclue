...
## 2026-08-28 - TTY Checks in interactive components\n**Learning:** Interactive TUI components often mistakenly use `isatty()` directly. In projects with abstracted runtime targets (like PSCAL here), using direct `isatty()` checks bypasses the project's own custom platform hooks (like `pscalRuntimeStdoutIsInteractive()`), which may be required to properly handle specific terminal emulation scenarios on platforms where standard TTYs are not available.\n**Action:** Always verify if a project has existing TTY/interactive abstraction methods (e.g., `pscalRuntimeStdoutIsInteractive`) before directly calling standard `isatty()`, especially when rendering interactive UI elements like the `top` or `watch` headers.
