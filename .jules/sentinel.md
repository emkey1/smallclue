...
## 2024-04-10 - Fix PATH injection vulnerability in su applet
**Vulnerability:** The `su` applet in `src/core.c` unset sensitive environment variables like `LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`, and `IFS`, but it failed to reset `PATH` to a safe default. This left the application vulnerable to PATH injection, allowing an attacker to execute malicious binaries if they controlled a directory early in the `PATH`.
**Learning:** Even if some variables are cleared, an untrusted `PATH` can be manipulated by an attacker to execute arbitrary code with elevated privileges during context transitions.
**Prevention:** Always reset `PATH` to a trusted, standard default (e.g., `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`) when clearing other environment variables to prevent privilege escalation via `execl` or `execvp`.
