...
## 2024-05-23 - [HIGH] Environment variable injection in su command
**Vulnerability:** The `su` command (`smallclueSuCommand` in `src/core.c`) did not correctly reset the `PATH` environment variable after sanitizing other variables before spawning the target shell.
**Learning:** Overlooking `PATH` reset in a setuid-like applet could allow privilege escalation by enabling an attacker to inject a malicious executable into the shell session by redefining `PATH` before invoking `su`.
**Prevention:** Always reset `PATH` to a standard safe default (`setenv("PATH", "/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin", 1)`) after unsetting potentially malicious variables when crossing privilege boundaries.
