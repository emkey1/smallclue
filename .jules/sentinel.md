...
## 2024-08-17 - Prevent Privilege Escalation via ENV and BASH_ENV in su/sudo
**Vulnerability:** Privilege escalation wrappers (`su` and `sudo`) were passing `ENV` and `BASH_ENV` environment variables when invoking shells or shell scripts. This allows a local attacker to execute arbitrary code with elevated privileges by placing malicious scripts in the path defined by these variables, which are automatically sourced by `sh` and `bash` respectively upon startup.
**Learning:** Even if `LD_PRELOAD` and `IFS` are sanitized, environment variables that control shell initialization scripts (`ENV`, `BASH_ENV`) represent a direct path to arbitrary command execution when traversing a privilege boundary. Standard practice requires an aggressive allowlist or an extensive blocklist of environment variables when changing user IDs.
**Prevention:** Always unset `ENV` and `BASH_ENV` in any C program or wrapper that performs `setuid()` / `setgid()` or invokes commands as another user (such as `sudo` or `su` implementations).
