...
## 2026-08-18 - Privilege Escalation via Shell Injection\n**Vulnerability:** Inherited `ENV` and `BASH_ENV` variables in `su` and `sudo` handlers allowed malicious shell scripts to be sourced upon startup during context transition.\n**Learning:** Privilege boundary transitions using `execv`/`execl` must rigorously sanitize environment variables that dictate shell initialization behavior, not just path-resolution variables like `LD_PRELOAD` and `PATH`.\n**Prevention:** Explicitly `unsetenv("ENV")` and `unsetenv("BASH_ENV")` alongside existing sanitizations prior to context switches.
