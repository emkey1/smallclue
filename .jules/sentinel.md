...
## 2024-05-25 - [CRITICAL] Environment variable injection in su and sudo commands (BASH_ENV/ENV)
**Vulnerability:** The `su` and `sudo` commands (`smallclueSuCommand` and `smallclueSudoCommand` in `src/core.c`) did not sanitize the `ENV` and `BASH_ENV` environment variables before spawning a shell or executing commands as root.
**Learning:** Shells like `sh` and `bash` evaluate the `ENV` or `BASH_ENV` environment variable upon startup to source initialization scripts. If an attacker controls these variables when transitioning privileges, they can achieve arbitrary code execution in the context of the elevated user (e.g., root).
**Prevention:** Always unset `ENV` and `BASH_ENV` along with other dangerous variables (like `LD_PRELOAD`, `IFS`) when cleaning up the environment for privilege-transitioning utilities.
