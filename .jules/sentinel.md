...
## 2024-05-14 - PATH Injection Vulnerability in su Applet
**Vulnerability:** The `su` applet sanitized standard dynamic loader environment variables (`LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`) and `IFS` but failed to reset `PATH` to a safe default. This allowed for potential privilege escalation via `PATH` injection if a malicious directory was prepended to the `PATH` environment variable by a user prior to executing the `su` command.
**Learning:** During context transitions, sanitizing known dangerous variables is insufficient. A complete reset of environment dependencies (like `PATH`) to safe, hardcoded defaults is required to guarantee a trusted execution environment.
**Prevention:** Always explicitly set `PATH` to a safe default list of directories (e.g., `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`) in addition to unsetting other sensitive environment variables when building tools that escalate privileges or switch user context.
