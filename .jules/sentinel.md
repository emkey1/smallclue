...
## 2024-05-24 - Missing PATH sanitization during context transition
**Vulnerability:** The `su` command transitions the execution context to a different user, potentially with elevated privileges (like root), but failed to sanitize the `PATH` environment variable. This allows PATH injection where an attacker could influence the lookup of dynamically executed binaries.
**Learning:** During context switches (e.g., using `execv`, `execl` after changing uids in `su` or `sudo`), sanitizing just library injection vectors like `LD_PRELOAD` or `LD_LIBRARY_PATH` is insufficient. The binary path search vector (`PATH`) must also be explicitly reset to a safe default, as the environment is inherited unless explicitly cleared.
**Prevention:** Always reset `PATH` to a safe default (e.g., `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`) along with other environment sanitization steps before invoking a shell or another binary in context-transitioning applets like `su` or `sudo`.
