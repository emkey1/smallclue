...
## 2024-05-24 - Unsetting ENV and BASH_ENV in Privilege Escalation Wrappers
**Vulnerability:** Shell wrappers that transition user context (`su`, `sudo`) without unsetting `ENV` and `BASH_ENV` allow attackers to execute arbitrary commands by setting these variables to point to malicious initialization scripts.
**Learning:** Standard C environment sanitization in elevated contexts often overlooks shell initialization variables (`ENV`, `BASH_ENV`), focusing only on `PATH`, `IFS`, and `LD_PRELOAD`.
**Prevention:** Always unset `ENV` and `BASH_ENV` alongside other environment variables before calling `execl`, `execv`, or executing commands in a new user context to prevent shell injection attacks.
