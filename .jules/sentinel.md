...
## 2024-12-07 - [Privilege Escalation via ENV/BASH_ENV]
**Vulnerability:** The `su` and `sudo` applets sanitized several dangerous environment variables (LD_PRELOAD, IFS) but failed to sanitize `ENV` and `BASH_ENV`. These variables can be used to inject shell profiles or scripts when transitioning user context, allowing a local attacker to execute arbitrary code with elevated privileges.
**Learning:** In setuid/privilege-boundary utilities, sanitizing `LD_*` and `IFS` is necessary but not sufficient. Modern shells (like bash, sh) aggressively source initialization scripts specified by `ENV` and `BASH_ENV` upon startup. If these are not unset, an attacker can control the execution flow of the target shell.
**Prevention:** Consistently unset `ENV` and `BASH_ENV` alongside other dangerous environment variables whenever transitioning user context (e.g., in `su`, `sudo`).
