...
## YYYY-MM-DD - [CRITICAL] Fix environment injection via ENV/BASH_ENV
**Vulnerability:** The `su` and `sudo` applets failed to unset the `ENV` and `BASH_ENV` environment variables when transitioning privileges. These variables can specify initialization scripts that are executed by the shell upon startup, potentially allowing a malicious user to achieve command execution in a privileged context.
**Learning:** Sanitizing common environment variables like `LD_PRELOAD`, `PATH`, and `IFS` is crucial, but variables specific to shell initialization (`ENV`, `BASH_ENV`) must also be explicitly neutralized. When crossing a privilege boundary and spawning a shell, the execution context must be fully restricted from user-controlled inputs.
**Prevention:** Always unset shell initialization environment variables such as `ENV` and `BASH_ENV`, in addition to `LD_*` and `PATH`, before invoking `execl` or transitioning privileges in applets like `su` and `sudo`.
