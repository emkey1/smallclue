...
## 2024-04-08 - Fix PATH Injection Vulnerability in su Applet
**Vulnerability:** The `su` applet did not reset the `PATH` environment variable when transitioning user context, making it vulnerable to PATH injection attacks where a malicious user could potentially execute arbitrary code with elevated privileges if a command relied on a relative path or an unsafe absolute path that was overridden by an injected PATH variable.
**Learning:** Even if `LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`, and `IFS` are unset, an untrusted `PATH` can still lead to privilege escalation when `execl` or `execv` are used to execute the shell or commands in the new context.
**Prevention:** Always reset the `PATH` environment variable to a safe, default absolute path (e.g., `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`) before calling `execl` or `execv` in applications that transition user context, ensuring that subsequent commands use known, trusted executables.
