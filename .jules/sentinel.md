...
## YYYY-MM-DD - [CRITICAL] Shell Initialization Injection in su/sudo Applets
**Vulnerability:** The `su` and `sudo` applets in `src/core.c` sanitized certain environment variables (`LD_PRELOAD`, `IFS`, etc.) but failed to unset `ENV` and `BASH_ENV` before executing commands as another user. If an attacker controlled these variables before invoking `su`/`sudo`, the target shell would source an attacker-controlled script upon initialization, leading to privilege escalation before the command is even executed.
**Learning:** Shells automatically source scripts defined in `ENV` (for POSIX sh) and `BASH_ENV` (for bash) upon startup, even non-interactive ones depending on the shell implementation. Leaving these variables intact across a privilege boundary allows arbitrary code execution in the context of the elevated user.
**Prevention:** Always unset shell initialization variables (`ENV`, `BASH_ENV`) in addition to library/loader variables when sanitizing the environment before calling `execl` or `execv` in applets that transition user context.
