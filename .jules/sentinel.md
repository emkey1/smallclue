...
## 2024-05-24 - [CRITICAL] PATH Injection in `su` Applet
...
## YYYY-MM-DD - [CRITICAL] Shell Injection via ENV and BASH_ENV in Privilege Transitions
**Vulnerability:** The `su` and `sudo` applets sanitized standard environment variables (like `LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`, `IFS`) but failed to unset `ENV` and `BASH_ENV` before executing commands across privilege boundaries. If the target shell respects these variables (e.g., executing `/bin/sh` or `/bin/bash`), an attacker could point them to a malicious script to execute arbitrary code with elevated privileges before the intended command runs.
**Learning:** Shell-specific startup scripts defined by `ENV` or `BASH_ENV` are powerful execution vectors that trigger automatically when a shell initializes. Merely sanitizing dynamic linker variables or `PATH` is insufficient if the target shell invokes attacker-controlled initialization scripts upon startup during a privilege transition.
**Prevention:** Consistently sanitize shell-specific execution hooks (`unsetenv("ENV");` and `unsetenv("BASH_ENV");`) in any setuid-like utility (`su`, `sudo`, `login`) that spawns shells or relies on `exec` across privilege boundaries.
