...
## 2024-11-20 - [Shell Injection in su/sudo Applets]
**Vulnerability:** The `su` and `sudo` applets sanitized some environment variables but left `ENV` and `BASH_ENV` intact before transitioning user context. This could allow privilege escalation via shell injection if an attacker pointed these variables to malicious scripts.
**Learning:** Always sanitize `ENV` and `BASH_ENV` alongside other dangerous variables like `LD_PRELOAD` and `IFS` before calling `execl` or `execv` in applets that transition user context.
**Prevention:** Unset `ENV` and `BASH_ENV` as part of the standard environment sanitization process for privileged applets.
