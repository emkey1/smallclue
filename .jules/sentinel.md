...
## 2024-08-26 - Prevent Privilege Escalation via Environment Injection in setuid Context
**Vulnerability:** The `su` and `sudo` equivalents were correctly unsetting `LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`, and `IFS` but failed to sanitize `ENV` and `BASH_ENV` prior to calling `execvp`/`system` and transitioning to a privileged execution context.
**Learning:** `ENV` (for `/bin/sh` or POSIX shells) and `BASH_ENV` (for `bash` running non-interactively) instruct the shell to source a script upon startup. An attacker can use this to execute arbitrary shell scripts with root privileges if a setuid binary invokes a shell or executes an application that in turn forks a shell. Environment sanitization logic in privilege-transition code must comprehensively block all shell initialization environment variables, not just library preloading variables.
**Prevention:** Always unset `ENV` and `BASH_ENV` alongside `LD_PRELOAD` and `IFS` before transitioning execution privileges, specifically when calling shell invocation equivalents (`system`, `/bin/sh -c`, `execl("/bin/sh")`).
