...
## 2024-05-24 - [CRITICAL] Shell Injection via ENV and BASH_ENV in Privilege-Boundary Applets
**Vulnerability:** The `su` and `sudo` applets sanitized standard dynamic linker variables (`LD_PRELOAD`, etc.) but failed to sanitize shell environment variables like `ENV` and `BASH_ENV`. When `execl` or `execv` spawns a shell (or a script using a shell), these variables can instruct the shell to run an attacker-controlled script before the intended command, leading to local privilege escalation.
**Learning:** Preventing privilege escalation requires sanitizing not just variables affecting the dynamic linker and executable path, but also variables that affect the behavior of the shells or interpreters commonly spawned as children (such as `sh` or `bash`).
**Prevention:** Always unset shell-specific injection variables (e.g., `ENV`, `BASH_ENV`) alongside `LD_*` and `IFS` before transitioning privileges and executing new commands.
