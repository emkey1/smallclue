...
## 2024-05-24 - [Environment Variable Injection in su Command]
**Vulnerability:** The `su` command (`smallclueSuCommand` in `src/core.c`) did not safely reset the `PATH` environment variable before executing the shell or command as the target user. This allows a user to invoke `su` with a malicious `PATH` setting to hijack executable lookups, leading to privilege escalation.
**Learning:** Any applet or function that crosses user privilege boundaries must completely sanitize the environment context it inherits, not just library-injection vectors like `LD_PRELOAD` or shell-manipulation vectors like `IFS`. `PATH` controls which binary gets executed, and failing to reset it gives control to the unprivileged caller.
**Prevention:** Always explicitly set `PATH` to a safe, trusted default (e.g., `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`) in addition to unsetting other sensitive environment variables when crossing privilege boundaries (e.g., `su`, `sudo`).
