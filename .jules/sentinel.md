...
## 2025-02-24 - [PATH Injection Vulnerability in su Applet]
**Vulnerability:** While `smallclueSuCommand` correctly unset variables like `LD_PRELOAD`, `LD_LIBRARY_PATH`, and `IFS`, it neglected to reset the `PATH` environment variable before executing a shell or command.
**Learning:** `PATH` manipulation is a classic privilege escalation technique. If a user sets their `PATH` to a directory they control, any commands executed by a script or program run with elevated privileges via `su` could inadvertently execute the user's malicious binaries instead of system tools. Unsetting variables is insufficient; variables that define execution lookup paths must be hardcoded to safe defaults.
**Prevention:** When sanitizing execution environments for context transitions (e.g., `su`, `sudo`), always reset `PATH` to a safe, absolute default (e.g., `setenv("PATH", "/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin", 1);`) to ensure subsequent command executions are deterministic and secure.
