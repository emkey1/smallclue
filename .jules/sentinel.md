...
## 2026-05-04 - Prevent Privilege Escalation via PATH Injection in su
**Vulnerability:** The `su` command was omitting a safe, explicit reset of the `PATH` environment variable during user context transition. It relied on the user's potentially compromised `PATH`.
**Learning:** Even though `sudo` safely reset the `PATH` variable along with unsetting `LD_PRELOAD` and friends, `su` missed the explicit reset, leaving a classic privilege escalation hole where an attacker could provide a malicious binary that `execl`/`execv` would resolve via the attacker-controlled `PATH`.
**Prevention:** Always reset `PATH` to a secure default list of standard system directories (e.g., `/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin`) whenever transitioning user contexts or calling `execl`/`execv` in setuid or root-oriented applets, directly alongside other environment sanitization procedures like unsetting `IFS` and `LD_*`.
