...
## 2024-11-20 - [CRITICAL] Shell Injection via ENV/BASH_ENV in Privilege Transition Applets
**Vulnerability:** The `su` and `sudo` applets sanitized some environment variables (`LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`, `IFS`) but failed to sanitize shell startup variables like `ENV` and `BASH_ENV`. If an attacker specifies a script in `BASH_ENV` or `ENV` and invokes a shell through these commands, the shell interprets these scripts with elevated privileges before execution, enabling local privilege escalation via shell injection.
**Learning:** For utilities acting as privilege boundaries, sanitizing only dynamic loader variables and `IFS` is not comprehensive enough. Because `su` and `sudo` often execute shells (`sh`, `bash`, `ash`) which inherently respect configuration variables like `ENV` or `BASH_ENV`, these variables also act as powerful command execution primitives during startup.
**Prevention:** Always unset `ENV` and `BASH_ENV` along with other unsafe environment variables when sanitizing the environment in any C applet that transitions user context (e.g., `su`, `sudo`) before calling `execl` or `execv`.
