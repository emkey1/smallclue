...
## 2024-11-20 - [CRITICAL] Shell Injection via ENV/BASH_ENV in su/sudo
**Vulnerability:** The `su` and `sudo` applets correctly sanitized `LD_PRELOAD`, `LD_LIBRARY_PATH`, `LD_DEBUG`, and `IFS`, and reset `PATH`, but missed sanitizing `ENV` and `BASH_ENV`. Certain shells, when spawned, will automatically source the file pointed to by these variables, enabling a local attacker to execute arbitrary commands as the target user (or root) by setting these variables before invoking the setuid binaries.
**Learning:** Privilege-transitioning wrappers must exhaustively scrub the environment of any variables that influence how child processes (especially shells) initialize themselves. Focusing only on dynamic linker variables and `PATH` is insufficient when the target executable is a shell interpreter.
**Prevention:** Always include `ENV` and `BASH_ENV` in the `unsetenv` sanitization block before executing `execv` or `execl` across a privilege boundary.
