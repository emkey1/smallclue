...
**Vulnerability:** The `su` and `sudo` applets sanitized several dangerous environment variables but failed to sanitize `ENV` and `BASH_ENV`.
**Learning:** These variables can be used to execute arbitrary commands when a shell is invoked, leading to local privilege escalation.
**Prevention:** Always sanitize `ENV` and `BASH_ENV` alongside other dangerous variables like `LD_PRELOAD` and `IFS` before crossing privilege boundaries.
