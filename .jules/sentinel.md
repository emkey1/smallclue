...
## 2026-08-20 - [CRITICAL] Shell Injection via ENV/BASH_ENV in Privilege Transition Applets
**Vulnerability:** The `su` and `sudo` applets sanitized variables like `LD_PRELOAD` and `IFS` but failed to unset `ENV` and `BASH_ENV`. This allowed local privilege escalation, as an attacker could set these variables to point to a malicious script, forcing the elevated shell to execute arbitrary commands upon startup.
**Learning:** Shell initialization scripts (`ENV` for POSIX sh, `BASH_ENV` for bash) are dangerous execution vectors when inheriting an attacker-controlled environment. Sanitizing library preloading and `PATH` is insufficient if the shell executes an untrusted script automatically before handing over control.
**Prevention:** Always unset `ENV` and `BASH_ENV` alongside other critical variables (`LD_PRELOAD`, `IFS`) when crossing privilege boundaries (e.g., in `su`, `sudo`, `login`) before executing a target shell or command.
