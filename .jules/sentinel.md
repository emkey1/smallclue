...
## 2024-05-24 - [Environment Injection in sudo/su via ENV/BASH_ENV]
**Vulnerability:** The `su` and `sudo` applets did not sanitize `ENV` or `BASH_ENV` environment variables before executing commands as root or another user. This allowed attackers to execute arbitrary shell scripts upon shell initialization, bypassing intended security boundaries.
**Learning:** Shells like bash and sh can execute scripts defined in `BASH_ENV` or `ENV` respectively upon startup. When crossing privilege boundaries (e.g., SUID binaries), these variables must be explicitly unset alongside `LD_*` and `IFS` to prevent arbitrary code execution via shell initialization files.
**Prevention:** Always unset `ENV` and `BASH_ENV` along with other dangerous environment variables (e.g., `LD_PRELOAD`, `IFS`) before executing commands in a privileged context.
