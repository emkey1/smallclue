...
## 2024-05-24 - Privilege Escalation via Shell Environment Injection in `su` and `sudo` wrappers
**Vulnerability:** The `su` and `sudo` applets implemented in `src/core.c` did not unset the `ENV` and `BASH_ENV` environment variables before transitioning context to the target user (e.g., root) and spawning a shell via `execl`.
**Learning:** Even if `LD_PRELOAD` and `PATH` are sanitized, POSIX shells (`sh`) and Bash automatically source scripts defined in `$ENV` and `$BASH_ENV` respectively upon startup. If these variables are passed through a setuid context transition without being cleared, an attacker can define them to point to an arbitrary malicious script, which the shell will run with the elevated privileges.
**Prevention:** Always explicitly `unsetenv("ENV")` and `unsetenv("BASH_ENV")` alongside `LD_PRELOAD`, `LD_LIBRARY_PATH`, and `IFS` when implementing command wrappers that transition user privileges and invoke standard shells.
