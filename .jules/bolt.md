...
**Learning:** For file stream utilities like `tee` that read from stdin and write to stdout and multiple files, wrapping the stream through stdio `fwrite()` imposes unneeded overhead due to buffering, memory copies, and lock acquisition compared to raw direct POSIX `write()` loops.
**Action:** Replace buffered `fopen`/`fwrite` with direct POSIX `open`/`write` in a loop that handles partial writes and `EINTR`, utilizing a 64KB stack buffer. `fflush(stdout)` must be called before transitioning to raw writes to prevent output interleaving. If a write to a specific output file or stdout fails, `tee` must record the error and continue writing to the remaining valid descriptors rather than aborting completely.
