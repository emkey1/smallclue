...
## 2024-07-18 - Ensure Target Branch is Active and Include Measurements
**Learning:** A PR proposing an optimization (like manual loop unrolling for `tr`) was deferred because it was accidentally opened against a frozen branch (`devel`) instead of `main`, meaning CI never ran. Additionally, because compilers often auto-vectorize and unroll loops, manual unrolling requires explicit before/after benchmarking to justify the added code complexity, particularly for shared code on multiple platforms.
**Action:** When creating performance PRs, always ensure you are targeting the active development branch (usually `main`), and always include before/after measurements in the PR description, especially when applying optimizations like loop unrolling that might duplicate what a compiler already does.
