...
## 2025-05-19 - Elimination of dynamic heap allocation in wide wc block processing loop
**Learning:** Bounded, short-lived multibyte processing blocks nested inside hot loops in stream utilities (like the `smallclueWcProcessFileWide` wide-character character counting loop) were dynamically allocating (`malloc()`) and freeing small amounts of memory per 16KB chunk read, imposing constant heap fragmentation and system call overhead despite strict maximum capacity boundaries (`sizeof(buf) + sizeof(carry)`).
**Action:** Replace `malloc`/`free` sequences inside deeply nested tight iterative reading loops with C99 variable-length arrays (`unsigned char scratch[sizeof(buf) + sizeof(carry)];`) on the stack when the upper bounds are definitively small and safe (e.g., <32KB).
