...
## 2024-05-23 - Optimize Tee throughput via syscall bypass
**Learning:** In the `tee` applet, using buffered `fopen`/`fwrite` adds memory copies and locking overhead, especially for high-throughput stream duplication. Directly wrapping raw POSIX `open`/`write` inside retry loops with a large stack buffer (64K) bypasses stdio entirely, boosting performance. Moreover, if a write to a specific output file or stdout fails, `tee` must record the error and continue writing to remaining valid descriptors rather than aborting completely.
**Action:** When implementing split-stream or duplicating utilities like `tee`, use unbuffered direct system calls with large stack buffers. Ensure `fflush()` is called on stdio streams before transitioning, handle partial writes / `EINTR`, and gracefully isolate failing file descriptors.
