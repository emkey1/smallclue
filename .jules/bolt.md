...
## 2025-05-20 - Optimize Cat throughput via syscall bypass
...
## 2025-05-21 - Optimization of wc wide character loop
**Learning:** In `src/core.c`, the wide-character processing loop in `smallclueWcProcessFileWide` is manually unrolled in the fast path (`smallclueWcProcessFileFast`), but the `smallclueWcProcessFileWide` function still uses a standard `for` loop over each character to count lines and words before doing wide character conversions. This inner byte loop causes branching overhead that slows down wide-character line/word counting.
**Action:** Unroll the initial byte processing loop in `smallclueWcProcessFileWide` by a factor of 16 just like in `smallclueWcProcessFileFast` to avoid branching overhead.
