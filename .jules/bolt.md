...
## 2026-07-31 - Optimization of smallclueTailStream ring buffer
**Learning:** For continuous stream operations like `tail`, replacing dynamically allocated strings in a ring buffer with a struct-based ring buffer (`{ char *data; size_t cap; }`) and passing its fields directly to buffer reading tools (`smallclueGetlineStream`) enables native capacity reuse via `realloc`. This drastically reduces the overhead of `malloc`, `memcpy`, and `free` for every line read.
**Action:** Always prefer native buffer capacity reuse strategies via `realloc` for continuous loop stream operations instead of performing redundant string allocations and copies.
