...
## 2025-05-19 - Optimization of tail stream buffering
**Learning:** The `tail` applet (`smallclueTailStream` in `src/core.c`) originally allocated a new string dynamically (`malloc`, `memcpy`) for every line processed, freeing the old one in a string array ring buffer. By changing this to a struct-based ring buffer containing `{ char *data; size_t cap; }` and passing those directly to `smallclueGetlineStream`, we reuse capacities natively via `realloc` and bypass constant dynamic allocation overhead per line.
**Action:** Replace arrays of dynamically allocated strings with struct-based buffers in continuous streaming/buffering routines to reuse capacities natively and reduce memory operation overhead.
