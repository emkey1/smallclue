...
## 2026-08-19 - Optimization of tail applet stream processing\n**Learning:** The `tail` applet originally implemented a ring buffer by doing dynamic memory allocation (`calloc`/`malloc`/`free`) for every single line in a file using `smallclueGetlineStream`. For very large files, allocating strings and executing memory copies imposes immense overhead that drags down throughput. \n**Action:** Replaced the array of dynamically allocated pointers with a predefined array of `TailBuffer` structs mapping `data` and `cap`. Supplying these struct attributes straight into `smallclueGetlineStream` allows native memory capacity reuse (`realloc`) to happen under the hood, significantly avoiding `memcpy`, `malloc`, and `free`.
