...
## YYYY-MM-DD - Optimize smallclueTailStream applet block allocations
**Learning:** The tail stream applet originally buffered strings line-by-line, causing it to dynamically allocate heap memory (malloc/free) and use memcpy for every single line of output stored in its circular buffer array `char **ring`. For long streams, these allocations drastically increased system overhead and decreased processing speed compared to reusing memory capacity directly.
**Action:** Replace arrays of dynamically allocated string pointers in continuous stream buffers with a struct-based ring buffer `struct TailBuffer { char *data; size_t cap; }`. Passing the struct pointers natively to `smallclueGetlineStream` allows it to natively reuse the same capacity under the hood using `realloc` instead of new mallocs and memcpys for every line.
