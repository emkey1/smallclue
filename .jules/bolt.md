...
## YYYY-MM-DD - Optimize tail stream block read
**Learning:** The `tail` applet (`smallclueTailStream` in `src/core.c`) is optimized by replacing an array of dynamically allocated strings with a struct-based ring buffer (`{ char *data; size_t cap; }`) and passing its fields directly to `smallclueGetlineStream` to reuse capacities natively via `realloc`, significantly reducing `malloc`, `memcpy`, and `free` overhead for continuous streams.
**Action:** For line-by-line streaming buffers like `tail`, use a struct array to cache and reuse `getline` line buffers, rather than allocating/copying/freeing on every iteration.
## 2025-05-19 - Failed tail stream capacity reuse optimization (Darwin/macOS getline quirk)
**Learning:** When attempting to avoid `malloc`/`memcpy`/`free` overhead in `tail` by reusing a ring buffer of struct elements passed directly to `getline`, it creates a silent output truncation bug on macOS/Darwin. At EOF, Darwin's `getline` implementation writes a NUL byte (`\0`) into the destination buffer before returning -1. If the destination buffer is a reused slot in the ring, this silently destroys the oldest valid line that still needed to be printed.
**Action:** For line-by-line streaming ring buffers, you must copy the successfully read string (`malloc` + `memcpy` + `free` old) rather than passing the cached array pointer directly into `getline`, as platform-specific `getline` implementations can mutate the destination buffer even on failure (EOF).
