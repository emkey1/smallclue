...
## YYYY-MM-DD - Optimization of wc wide-character processing path
**Learning:** In the `smallclueWcProcessFileWide` function of the `wc` applet, using dynamic memory allocation (`malloc`/`free`) per 16KB read block iteration introduced unnecessary heap management overhead. Furthermore, the character-by-character scan loop had high branching overhead.
**Action:** Unroll the initial byte scanning loop by a factor of 16 (matching the fast path optimization) to reduce branch evaluation overhead. Replace the dynamic `malloc` buffer with a fixed-size stack array (e.g., `unsigned char scratch[sizeof(buf) + sizeof(carry)];`) to eliminate memory management overhead in the hot file-processing loop.
