...
## 2025-10-23 - Optimize tail line allocation overhead
**Learning:** `tail` traditionally allocated memory line by line with `malloc`, `memcpy`, and `free` logic inside its circular buffer, creating unnecessary system call overhead on large continuous streams.
**Action:** Replace arrays of string pointers with a ring buffer of struct elements (`{ char *data; size_t cap; }`) and pass their fields directly into `getline`-style functions (`smallclueGetlineStream`), so capacity is natively reused via `realloc`, dropping inner-loop allocations to near zero.
