...
## YYYY-MM-DD - Optimize tail stream memory reuse
**Learning:** `smallclueTailStream` originally processed output by manually allocating and freeing strings on a dynamically sized `char **` ring buffer. In a hot tight loop streaming high output, repeated dynamic memory allocation via `malloc` and `free` operations creates heavy allocation overhead.
**Action:** Replace `char **` ring arrays with a struct-based buffer containing `{ char *data; size_t cap; }` and pass those directly to bounded read functions like `smallclueGetlineStream`. This reuses string capacities inherently through `getline`/`realloc` under the hood, preventing excessive allocations on continuous streams.
