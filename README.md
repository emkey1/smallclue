...
* **nslookup** / **host**: DNS lookup utilities; IP-shaped queries auto-detect as PTR/reverse lookups. An optional trailing `server` argument queries that DNS server directly over UDP/53 via a from-scratch DNS client (raw wire-format encode/decode, name compression, A/AAAA/PTR/CNAME), instead of going through the system resolver.
